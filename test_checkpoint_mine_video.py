...
def main():
    ...
    hparams = get_opts()
    # Ensure that we're in validation (rendering) mode and a checkpoint path is provided
    hparams.val_only = True
    if not hparams.weight_path:
        raise ValueError("Please provide a checkpoint path using --weight_path.")
    
    # Instantiate the system and move to device
    ...
    # Create directory for saving frames and video
    output_dir = "rendered_frames_vid"
    os.makedirs(output_dir, exist_ok=True)
    rendered_frames = []
    
    # Define the number of frames for a clear video
    num_frames = 150  # 150 frames = 5 seconds at 30 fps
    
    # Render frames from the test set until we reach num_frames
    for i, batch in enumerate(test_loader):
        ...
        frame_fname = os.path.join(output_dir, f"frame_{i:03d}.png")
        imageio.imsave(frame_fname, rgb_image)
        print(f"Frame {i} saved to {frame_fname}")
        
        rendered_frames.append(rgb_image)
    
    # Save the collected frames as a video at 30 frames per second
    video_fname = os.path.join(output_dir, "rendered_video.mp4")
    imageio.mimsave(video_fname, rendered_frames, fps=30)
    print(f"Rendered video saved to {video_fname}")
