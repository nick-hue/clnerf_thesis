...
def main():
    # Parse command-line arguments (use the opt.py from the repo)
    hparams = get_opts()
    # Ensure that we're in validation mode (rendering) and a checkpoint path is provided
    ...
    # Rearrange the predicted RGB output into an image.
    w, h = system.test_dataset.img_wh
    rgb_image = rearrange(results['rgb'], '(h w) c -> h w c', h=h)
    rgb_image = (rgb_image.cpu().numpy() * 255).astype(np.uint8)
    
    # Save the rendered image
    out_fname = "check_render2.png"
    imageio.imsave(out_fname, rgb_image)
    print(f"Rendered image saved to {out_fname}")
